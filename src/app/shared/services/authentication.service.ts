...
import {changeUserPass, Login, SignUp, UserMe,} from '../interfaces/authentication.interface';
...
export class AuthenticationService {
  ...
  checkLogin(): Observable<UserMe> {
    return this.http.get<UserMe>(`${main_url}accounts/me/`).pipe(
      tap((val) => {
        this.loadingService.loading.next(false);
      })
    );
  ...
  changeUserPassword(payload: changeUserPass): Observable<{ detail: string }> {
    return this.http
      .post<{ detail: string }>(
        main_url + 'dj-rest-auth/password/change/',
        payload
      )
      .pipe(
        tap((val) => {
